--[[
********************************************************************************
*                             Weekly Hunts Doer                                *
********************************************************************************

Description: Picks up weekly hunts from each expac and attempts to do them. It
will do the weekly Elite hunts.

   
********************************************************************************
*                                 Version 1.0.0                                *
********************************************************************************
    1.0.2   Fixed 3-star hunt pickups
            Fixed n+1 hunt requirement

********************************************************************************
*                               Required Plugins                               *
********************************************************************************

2. Wrath Combo
3. Bossmod (VBM)

********************************************************************************
*                                    Settings                                  *
********************************************************************************
--]]
yield("/xlenableplugin Wrath Combo")
yield("/xlenableplugin Hunt Buddy")
yield("/wait 5")

ARR = true
HW = true   -- In case you do not want to do a particular board, you can switch it to false here.
SB = true
SHB = true
EW = true
DT = true

--[[ 
********************************************************************************
*           Code: Don't touch this unless you know what you're doing           *
********************************************************************************
--]]

skipBoard =
{
	{
		status = ARR
	},
	{
		status = HW
	},
	{
		status = SB
	},
	{
		status = SHB
	},
	{
		status = EW
	},
	{
		status = DT
	},
}
HuntBoards =
{
	{
		city = "Limsa Lominsa",
		zoneId = 129,
		aetheryte = "Limsa Lominsa Lower Decks",
		miniAethernet = {
			name = "The Aftcastle",
			x=16, y=41, z=69
		},
		boardName = "Hunt Board",
		bills = 2001362,
		x=94, y=40, z=60,
	},
	{
	        city = "Ishgard",
	        zoneId = 418,
	        aetheryte = "Foundation",
	        miniAethernet = {
	            name = "Forgotten Knight",
	            x=45, y=24, z=0
	        },
	        boardName = "Clan Hunt Board",
	        bills = 2001703,
	        x=73, y=24, z=22,
    	},
	{
   	     	city = "Kugane",
	        zoneId = 628,
	        aetheryte = "Kugane",
	        boardName = "Clan Hunt Board",
	        bills = 2002116,
	        x=-32, y=0, z=-44
    	},
    	{
	        city = "Crystarium",
	        zoneId = 819,
	        aetheryte = "The Crystarium",
	        miniAethernet = {
	            name = "Temenos Rookery",
	            x=-108, y=-1, z=-59
	        },
	        boardName = "Nuts Board",
	        bills = 2002631,
	        x=-84, y=-1, z=-91
    	},
   	{
	        city = "Old Sharlayan",
	        zoneId = 962,
	        aetheryte = "Old Sharlayan",
	        miniAethernet = {
	            name = "Scholar's Harbor",
	            x=16, y=-17, z=127
	        },
	        boardName = "Guildship Hunt Board",
	        bills = 2003093,
	        x=29, y=-16, z=98
	},
    	{
	        city = "Tuliyollal",
	        zoneId = 1185,
	        aetheryte = "Tuliyollal",
	        miniAethernet = {
	            name = "Bayside Bevy Marketplace",
	            x=-15, y=-11, z=135
	        },
	        boardName = "Hunt Board",
	        bills = 2003513,
	        x=25, y=-15, z=135
    	}
}

-- #region Movement

-- Hunt Visland Scripts
wachu1 = ""
koza1 = ""
koza2 = ""
koza3 = ""
yak1 = ""
yak2 = ""
shaa1 = ""
shaa2 = ""
hf1 = ""
hf2 = ""
lm1 = ""
lm2 = ""
--EW
lab1 = ""
lab2 = ""
thav = ""
garl = ""
moon = ""
elp = ""
space1 = ""
space2 = ""
space3 = ""
--ShB
lake = ""
khol = ""
ahn = ""
mheg = ""
rak1 = ""
rak2 = ""
temp = ""
--SB
fringe1 = ""
fringe2 = ""
Buckaboo = ""
lochs = ""
ruby = ""
yanx = ""
azim = ""
bRank = {
	{name="Buccaboo", location="Ala Gannha", path=""},
	{name="Indomitable",location="Stilltide", path=""},
	{name="Iravati", location="Yedlihmad", path=""},
	{name="13th Child", location="Leynode Mnemo", path=""},
	{name="Go'ozoabek'be", location="Many Fires", path=""},
	{name="Ovjang", location="Black Brush Station", path=""},
}
bRank.SkogsFru = ""
bRank.Barbasetelle = ""
bRank.BloodyMary = ""
bRank.Myradrosh = ""
bRank.Vuokho = ""
bRank.DarkHelmet = ""
bRank.StingingSophie = ""
bRank.Buccaboo = ""
bRank.Thextera = ""
bRank.

-- #Hunt Visland Scripts

function TeleportTo(aetheryteName)
    yield("/tp "..aetheryteName)
    yield("/wait 1") -- wait for casting to begin
    while GetCharacterCondition(27) do
        yield("/wait 1")
    end
    yield("/wait 1") -- wait for that microsecond in between the cast finishing and the transition beginning
    while GetCharacterCondition(45) do
        yield("/wait 1")
    end
    yield("/wait 1")
end

function GoToDravanianHinterlands()
	while not IsInZone(478) do
		yield('/wait 1')
	end
	yield('/wait 3')
	if not GetCharacterCondition(4) then
		yield('/gaction "mount roulette"')
	end
	yield('/wait 4')
        PathfindAndMoveTo(148.51, 207.0, 118.47)	
end


-- #endregion Movement

-- #region Hunt

function GoToHuntBoard(BoardNumber)
	Board = HuntBoards[BoardNumber]
	if GetItemCount(Board.bills) == 1 then
		yield('/echo Skipping Pickup, we have the bill...')
        	return 1
	elseif not IsInZone(Board.zoneId) then
		yield('/echo Bill missing! We need to get to the zone.')
	        TeleportTo(Board.aetheryte)
		repeat
			yield('/wait 1')
		until IsInZone(Board.zoneId)
	end
	yield ('/wait 2')
	yield ('/echo Zone found, checking distance to the board to see if we need to use the aethernet to get there.')
	if Board.miniAethernet ~= nil and GetDistanceToPoint(Board.x, Board.y, Board.z) > (DistanceBetween(Board.miniAethernet.x, Board.miniAethernet.y, Board.miniAethernet.z, Board.x, Board.y, Board.z) + 20) then
	        yield("/target aetheryte")
	        yield("/wait 0.5")
		yield('/echo Aethernet required, moving to the aetherite and zoning.')
		if GetDistanceToTarget() > 10 then
			PathMoveTo(GetTargetRawXPos(), GetTargetRawYPos(), GetTargetRawZPos())
			repeat
		    		yield('/wait 0.1')
			until GetDistanceToTarget() < 10
		end
		yield("/vnav stop")
	        
	        yield("/li "..Board.miniAethernet.name)
	        yield("/wait 5")
    	end

	if IsAddonVisible("TelepotTown") then
	        yield("/callback TelepotTown true -1")
	end
	yield('/echo Moving to the board.')
	if GetDistanceToPoint(Board.x, Board.y, Board.z) > 3 then
	        PathfindAndMoveTo(Board.x, Board.y, Board.z)
	repeat
		yield('/wait 0.5')
	until GetDistanceToPoint(Board.x, Board.y, Board.z) <= 3
	yield("/vnav stop")
	end
end


function PickUpHunts(BoardNumber)
yield("/target "..Board.boardName)
yield("/wait 0.5")
yield("/interact")
yield("/wait 0.5")
while IsAddonVisible("SelectString") == false do
yield("/wait 0.5")
end
if BoardNumber == 1 then
yield("/callback SelectString true 1")
yield("/wait 0.5")

while IsAddonVisible("Mobhunt") == false do
yield("/wait 0.5")
end
local callback = "/callback Mobhunt true 0"
    yield(callback)
else
yield("/callback SelectString true 3")
yield("/wait 0.5")
while IsAddonVisible("Mobhunt"..BoardNumber) == false do
yield("/wait 0.5")
end
local callback = "/callback Mobhunt"..BoardNumber .." true 0"
    yield(callback)
end

yield("/wait 0.5")
if IsAddonVisible("SelectYesno") and GetNodeText("SelectYesno", 15) == "Pursuing a new mark will result in the abandonment of your current one. Proceed?" then
yield("/callback SelectYesno true 1")
end
end

function ParseHuntChat() -- Pattern to match the quantity and item name
    local chat = GetNodeText("ChatLogPanel_3", 7, 2)
    -- Pattern to match the quantity, item name, and location
    local huntingPattern = "Hunting elite mark ([%w%s%-'–]+) in ([%w%s%-'–]+)"
    
    -- Find all matches and store them in a table
    local hunt = nil
    for line in chat:gmatch("[^\r\n]+") do
        for markName, location in line:gmatch(huntingPattern) do
            hunt = {name = markName, location = location}
        end
    end
    return hunt
end

function GoToZone(hunt)

    if hunt == "Idyllshire" then
        GoToDravanianHinterlands()
    else
        TeleportTo(hunt)
    end
end


function DoHunt(hunt)
	yield('/target '..hunt)
	yield("/vbm ai on")
    yield("/wrath auto on")

	if GetDistanceToTarget() > 5 + GetTargetHitboxRadius() then
        	PathfindAndMoveTo(GetTargetRawXPos(), GetTargetRawYPos(), GetTargetRawZPos())
		yield("/wait 1")
		while GetDistanceToTarget() > 5 + GetTargetHitboxRadius() do
			yield("/wait 0.2")
		end
	end
	while not GetCharacterCondition(26) do
		yield('/send KEY_1')
		yield('/wait 3')
	end

    	while GetCharacterCondition(26) do
            yield("/wait 1")
	end
end

-- #endregion Hunt



for i=1,6,1 do
	yield('/echo Starting Loop '..i..' of 6, checking to see if this loop is set to false.')
	skipStatus = skipBoard[i]
	if not skipStatus.status == true then
		yield('/wait 3')
		yield('/echo Skipping this board - we have opted not to hunt this target.')
	else 
		yield('/wait 0.5')
		yield('/echo Hunt Board '..i..' set to true, checking to see if we need to go to pick up the bill.')
		skipPickup = GoToHuntBoard(i)
		if skipPickup ~= 1 then
			yield ('/wait 1')
			PickUpHunts(i)
		else
			yield('/echo Bill found, skipping pickup.')
		end
		yield ("/wait 1")
		yield ("/phb next")
		yield ("/wait 0.5")
		yield ('/echo Checking for hunt string.')
		hunt = ParseHuntChat()
		yield ('/echo Hunt found. Hunt Name: '..hunt.name)
		yield ("/wait 0.5")
		for _,monsters in pairs(bRank) do
			yield('/echo Mob name: '..monsters.name)
			if monsters.name == hunt.name then 
				huntLocation = monsters.location
				huntPath = monsters.path
				yield ('/echo Hunt located. Target: '..monsters.name..' located near aetheryte: '..huntLocation)
				break
			end
		end
		GoToZone(huntLocation)
		yield ("/wait 0.5")
		if not GetCharacterCondition(4) then
			yield('/gaction "mount roulette"')
		end
		exitLoop = false
		VislandStartRoute(huntPath, true)
		yield ("/wait 5")
		while IsVislandRouteRunning() do
			if DoesObjectExist(hunt.name) then exitLoop = true end
			if exitLoop then
				yield('/wait 3')
				yield('/visland stop')
			end
			yield("/wait 1")
		end
		while GetCharacterCondition(4) do
			yield('/ac dismount')
			yield('/wait 1')
		end
		yield('/wait 1')
		yield('/target '.. hunt.name)
		DoHunt(hunt.name)
		yield('/wait 5')
		yield('/wrath auto off')
		yield('/vbm ai off')
	end
end
yield("/xldisableplugin Hunt Buddy")
